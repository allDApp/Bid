		...
		nebpay.simulateCall(to, value, callFunction, callArgs, {
			listener: function (resp) {
				...
				for (var i = 0; i < length; i++) {
					...
					tempStr += '<h5>[' +res[i].index +']  &nbsp;&nbsp;&nbsp;'+ res[i].info + '</h5>'
					tempStr += '<p><br>';
...
tempStr += '<br><h6><p class="star">*</p> Item Details</h6>';
...
tempStr += '<input type="text" class="form-control" rows="1" id="contact" placeholder="Email: Tel: Loc:">';
...
tempStr += '</div>';
	...
	$("#help").click(function () {
		...
		tempStr += '<p>How to use it?</p><br>';
		tempStr += '<p>Need WebExtensionWallet:</p><br>';
...
tempStr += "Serial number:" + res[i].index + "<br> 	Details:" + res[i].info ;
tempStr += '</div>';
tempStr += '<br><br>';
...
tempStr += '<small>' +'  Seller Wallet:' + '</small> ';
tempStr += '</div>';
tempStr += '<hr>';
...
function save() {
	...
	texts = name+' '+content +'--'+contact

	var to = dappAddress;
	var value = "0.00001000001";//保证金
	var callFunction = "newo";
	var callArgs = '["' + texts + '",' + time + ']';
...
function conbid(i) {
	...
	nebpay.call(dappAddress, "0", "con", "[\"" + ids + "\"]", {
		listener: function Push(resp) {
			...
			var respString = JSON.stringify(resp);
			if (respString.search("rejected by user") !== -1) {
				alert("Close transaction, cancel")
			} else if (respString.search("txhash") !== -1) {
		...
		listener: function (resp) {
			...
			if (j.length !== 0) {
				for (var z = 0; z < j.length; z++) {
					var c = j[z];
					if(res[c].end !== ''){
						pdpm += '<button class="button3" class="btn btn-primary" id="conbidbutton" onclick="alert("Auction has ended");">'+'Deal done    Time:'+ttt(res[c].end)+'   Buyer:'+res[c].author+'</button>&nbsp;';
					}
